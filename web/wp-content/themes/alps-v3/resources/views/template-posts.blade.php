...
@section('content')
  ...
  <div>
    ...
    @include('patterns.02-organisms.sections.page-header-feature-post')
  </div>
  @php(the_content())
  <section class="l-grid l-grid--7-col l-grid-wrap--6-of-7 u-spacing--double--until-large">
    <article class="c-article l-grid-item l-grid-item--l--4-col u-padding--zero--sides">
      @if ($post_feed_link != 'post_feed_link_false')
        <div class="c-block-wrap u-spacing--double">
          @if ($post_feed_link_title || $post_feed_link_link)
            <div class="c-block__heading u-theme--border-color--darker">
              @if ($post_feed_link_title)
                <h3 class="c-block__heading-title u-theme--color--darker">{{ $post_feed_link_title }}</h3>
              @endif
              @if ($post_feed_link_link)
                <a href="{{ $post_feed_link_link }}" class="c-block__heading-link u-theme--color--base u-theme--link-hover--dark">See All</a>
              @endif
            </div>
          @endif
          @php
            if ($post_feed_link_count) {
              $post_feed_link_count = $post_feed_link_count;
            } else {
              $post_feed_link_count = 5;
            }
            $posts = new WP_Query(array(
              'post_type' => 'post',
              'posts_per_page' => $post_feed_link_count,
              'post_status' => 'publish',
              'category__in' => $post_feed_link_category,
              'order_by' => 'date'
            ));
          @endphp
          @if ($posts->have_posts())
          ...
          @endif
          @if ($post_feed_link_link)
            <a href="{{ $post_feed_link_link }}" class="o-button o-button--outline o-button--with-arrow u-space--left">See All<span class="u-icon u-icon--arrow--long u-theme--path-fill--base u-space--half--right">@include('patterns.00-atoms.icons.icon-arrow-long-right')</span></a>
          @endif
        </div>
      @endif
    </article>
