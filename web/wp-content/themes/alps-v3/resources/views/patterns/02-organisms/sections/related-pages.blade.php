@php
  ...
  $related_layout = get_post_meta($post->ID, 'related_layout', true);
  ...
  <div class="u-spacing--double u-space--double--top">
      ...
      if ($related_layout == "grid") {
        if (get_post_meta($post->ID, 'hide_sidebar', true) == 'true') {
          if (get_post_meta($post->ID, 'related_grid_3up', true) == 'true') {
            ...
            $grid_class = "l-grid-item--6-col l-grid-item--m--4-col u-shift--left--1-col--standard u-no-gutters";
    ...
    @endphp
      @foreach ($pages as $page)
        ...
        @endphp
        @if ($related_layout == "grid")<div class="l-grid-item {{ $grid_item_class }}">@endif
          @if ($thumb_id)
            @include('patterns.01-molecules.blocks.media-block')
          @else
            @include('patterns.01-molecules.blocks.content-block')
          @endif
        @if ($related_layout == "grid")</div>@endif
      @endforeach
    @if ($related_layout == "grid")</div>@endif
  </div>
@endif
